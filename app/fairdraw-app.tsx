...
export function FairDrawApp({ member, initialTickets }: { member: MemberProfile; initialTickets: MemberTicket[] }) {
  ...
  async function generateDemoTicket() {
    const response = await fetch("/api/tickets", {
      ...
      body: JSON.stringify({ ticketPrice: selectedPrice, quantity }),
    });
    if (!response.ok) return;
    const data = await response.json();
    const first = data.tickets[0]?.ticketCode;
    setDemoTicket(data.tickets.length > 1 ? `${first} + ${data.tickets.length - 1} more` : first);
    window.dispatchEvent(new Event("fairdraw:ticket-created"));
  }
    ...
    <main className="app-shell">
      ...
      <section className="purchase-panel" aria-labelledby="purchase-title">
        ...
        <div className="purchase-controls">
          ...
          <Button size="lg" className="buy-button" onClick={() => void generateDemoTicket()}>
